...
class IntroState extends BaseState {
  ...
  enter() {
    ...
    // Decoración dentro del ancho vertical
    this.decorItems = [
      this._makeDecor(140, 380, 0.9),
      this._makeDecor(940, 420, 1.1),
      this._makeDecor(120, 1280, 0.8),
      this._makeDecor(960, 1240, 1.0),
    ];

    this.logo.alpha = 0;
    this.logo.y = 160;
    this.logo.scale = 0.85;
  ...
  _makeDecor(x, y, scale) {
    return {
      ...
      alpha: 0,
      rotation: 0,
      imageKey: Math.random() < 0.5 ? 'decor_star' : 'decor_spark',
  ...
  _playEnterAnimations() {
    ...
    tw.animate(this.logo, { alpha: 1, y: 220, scale: 1 }, 0.55, {
  ...
  _playExitAnimations(onComplete) {
    ...
    tw.animate(this.logo, { alpha: 0, y: 140, scale: 0.9 }, 0.35, {
